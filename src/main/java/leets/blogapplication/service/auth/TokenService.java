...
public class TokenService {
    ...
    public TokenService(TokenProvider tokenProvider, RefreshTokenService refreshTokenService,
                        UserRepository accountRepository, RefreshTokenRepository refreshTokenRepository, UserSocialRepository userSocialRepository) {
    ...
    public String createNewAccessTokenSocial(String refreshToken) {
        if(!tokenProvider.validToken(refreshToken)) {
            throw new IllegalArgumentException("Invalid refresh token");
        }

        Long userId = refreshTokenService.findByRefreshToken(refreshToken).getSocialUserId();
        UserSocial user = userSocialRepository.findById(userId)
                .orElseThrow(() -> new IllegalArgumentException("access Account not found"));

        return tokenProvider.generateAccessToken(userId, Duration.ofHours(2));
        //첫 argument로 들어가는 놈은 무조건 extends UserDetails를 한 놈만 됨
    }
    ...
    public String createNewRefreshToken(Long userId) {
        UserSocial user = userSocialRepository.findByProviderUserId(userId)
                .orElseThrow(() -> new IllegalArgumentException("refresh Account not found"));
        String ref = tokenProvider.generateRefreshToken(user.getId(), Duration.ofDays(1));
        refreshTokenRepository.save(RefreshToken.createRefreshToken(ref, user));
        return ref;
    }
